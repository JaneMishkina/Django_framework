...
# Пользовательская валидация данных с помощью метода clean()
    def clean_name(self):
        """Плохой пример. Подмена параметра min_length.
           С одной стороны: корректный подход - записать min_length выше в определении формы name.
           С другой стороны: если необходимо изменить стандартный текст при выводе ошибки, то можно использовать и такой вариант. """
...
# Создадим форму для демонстрации разнообразия полей:
